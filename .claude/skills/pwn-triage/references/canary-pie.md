...
Use this reference after initial recon when the binary has canary and/or PIE enabled and the vulnerability is still stack-based. It fills the gap before stack-overflow exploit development by determining how to leak the values that the basic scaffold assumes are absent.
...
- Initial recon reports canary and/or PIE enabled
...
- Neither canary nor PIE is enabled -- go straight to stack-overflow reference
...
python3 src/tools/pwn/leak_scaffold.py \
...
Decision: if the only viable primitive is partial overwrite and it reaches a win function, hand off to stack-overflow reference with the partial overwrite approach.
...
1. Run initial recon and confirm canary/PIE in `analysis.json`.
...
6. Once the leak is stable, the payload section follows stack-overflow patterns.
...
- the vulnerability is heap-based -- wrong reference entirely
- partial overwrite alone reaches a win function -- hand off to stack-overflow reference
