...
from tools.common.models import ChallengeInfo
from tools.workspace.workspace import WorkspaceManager
...
class WorkspaceManagerTest(unittest.TestCase):
    ...
    def test_init_script_creates_workspace(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            script = Path(__file__).resolve().parents[1] / "src" / "tools" / "workspace" / "init_workspace.py"
