...
Use this workflow when evidence points to client-side code execution through reflected or DOM-based XSS.
...
```bash
python3 src/tools/web/xss_probe.py \
...
```bash
python3 src/tools/web/browser_ctl.py \
  xss-test \
...
- `src/tools/web/browser_ctl.py xss-test`
