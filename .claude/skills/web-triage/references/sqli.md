...
Use this workflow when evidence points to SQL injection.
...
```bash
python3 src/tools/web/boolean_probe.py \
...
```bash
python3 src/tools/web/sqlite_boolean_extract.py int \
  --target http://127.0.0.1:5001/search \
  --param id \
  --prefix "1 AND " \
...
```bash
python3 src/tools/web/sqlite_boolean_extract.py string \
  --target http://127.0.0.1:5001/search \
  --param id \
  --prefix "1 AND " \
...
Keep this workflow focused on first-pass triage and the shortest reliable blind-extraction path.
