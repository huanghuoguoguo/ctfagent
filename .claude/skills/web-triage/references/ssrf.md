...
Use this workflow for CTF web challenges where one user-controlled URL may unlock:
...
Use `fetch_target.py` for all routine requests. It keeps the command shape consistent and extracts the `<textarea>` content when the page wraps results in HTML.

Typical sequence:

```bash
python3 src/tools/web/fetch_target.py \
...
Keep this workflow lean and only add branches that were observed in real targets.
